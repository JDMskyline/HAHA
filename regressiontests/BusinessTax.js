...
let ssn = config.Biz_EIN;
let emailAdd = config.AD_USER_NAME;
let mailingAddress = config.AD_MAILING_ADDRESS;
// mail | email
...
let principalamt = 98800;
let interestamt = 100;
let penaltyamt = 300;
let commentTest = config.commentTest;
let businessName = config.businessName;
let caseDescription = config.commentTest;
//Spanish | Vietnamese | French  (need to add none for no interpreter)
let InterpreterLang = "Vietnamese";
...
//describe block
describe("TRB | Business Tax - Test Started", function () {
  //it block

  it("Business Tax - Test Automation", async function () {
    //non-headless----------------------------------------------------
    /*
...
*/
    ...
    switch (InterpreterLang) {
      ...
      case "Vietnamese":
        await driver
          .findElement(By.xpath("//*[contains(text(),'Vietnamese')]"))
          .click();
        break;
      case "French":
        await driver
    ...
    await driver
      .findElement(By.xpath("//*[@name='hearingComments']"))
...
*/
    ...
    await driver
      .findElement(By.xpath("//input[@name='customerNumber']"))
      .sendKeys(ssn);
    ...
    hearingDate.setDate(currentDate.getDate() + 0); //---
